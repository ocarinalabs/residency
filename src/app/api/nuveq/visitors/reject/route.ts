...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`Rejecting visitor: ${name || credential}`);
    ...
    // Click the Reject button in that row
    // Based on the actual HTML, the reject button has class "bg-error"
    const rejectButton = rowLocator.locator("button.bg-error").first();

    // Check if button exists
    if ((await rejectButton.count()) === 0) {
      console.log("Reject button not found. Available buttons:");
      const buttons = await rowLocator.locator("button").all();
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Reject button not found for this visitor",
        },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Visitor ${name || credential} rejected successfully`,
    });
  } catch (error) {
