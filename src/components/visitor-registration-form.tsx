...
import { mockRooms } from "@/lib/room-data";
...
export function VisitorRegistrationForm() {
  ...
  const handleFormComplete = useCallback(() => {
    ...
    form.reset();
    setIsLoading(false);
    // Keep the iframe visible and data intact so user can see the response
    // setSubmitData(null);
    // setShowIframe(false);
  }, [form]);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      toast.info("Registration submitted. Check your email for confirmation.");
      // Don't reset the form or clear data - keep iframe visible
      // form.reset();
      // setSubmitData(null);
    }, 3000);
  }
    ...
    <div className="w-full max-w-md mx-auto p-6 bg-card rounded-lg border">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>IC/Passport Number</FormLabel>
                <FormControl>
                  <Input
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Visit ends at 11:59 PM on the selected date
            </p>
          ...
          <div className="space-y-4 pt-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Choose a meeting room for your visit
              </p>
            </div>
              ...
              {mockRooms.slice(0, 6).map((room) => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedRoom === room.id && "ring-2 ring-primary rounded-lg"
                  )}
                  onClick={() => {
                    setSelectedRoom(room.id);
                    form.setValue("selectedRoom", room.id);
                    // Update reason to visit with room name
                    const currentReason = form.getValues("reasonToVisit");
                    if (!currentReason.includes(room.name)) {
                      form.setValue(
                        "reasonToVisit",
                        `${currentReason} - ${room.name}`
                      );
                    }
                  }}
                >
      ...
      <div className="mt-4 flex items-center space-x-2">
        <input
          ...
          className="h-4 w-4"
        />
        <label htmlFor="show-iframe" className="text-sm text-muted-foreground">
